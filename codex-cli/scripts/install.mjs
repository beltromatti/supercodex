...
if (platform === "darwin" && arch === "arm64") {
  ...
  console.error(
    `[super-codex] Unsupported platform ${platform} (${arch}). ` +
...
function download(targetUrl, destPath, redirectsLeft = 5) {
  return new Promise((resolve, reject) => {
    https
      .get(targetUrl, { headers: { "user-agent": "super-codex-postinstall" } }, (res) => {
        if (
          res.statusCode &&
          res.statusCode >= 300 &&
          res.statusCode < 400 &&
...
console.log(`[super-codex] Downloading ${assetName}...`);
try {
  await download(url, outPath);
  if (!isWindows) {
    chmodSync(outPath, 0o755);
  }
  console.log(`[super-codex] Installed binary at ${outPath}`);
} catch (err) {
  console.error(
    `[super-codex] Failed to download ${url}: ${err.message || err}`,
  );
  console.error(
    `[super-codex] You can download it manually from ` +
