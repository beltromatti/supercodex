...
pub fn model_info_from_slug(slug: &str) -> ModelInfo {
    ...
    // Super Codex: the self-hosted Qwen3-VL AWQ deployment does not ship
    // in the upstream `models.json` catalog, so a manual picker / config
    // reference can still land here. Return a hand-built descriptor with
    // the real display name + context window instead of the generic
    // 272k-token fallback.
